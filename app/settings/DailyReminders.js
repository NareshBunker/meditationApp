...
const DailyReminders = () => {
  ...
  // Load Reminders and filter out past reminders
  ...
  // Lifecycle hook to load initial configuration data
  useEffect(() => {
    requestPermissions();
    loadUserDetails();
    loadReminders();
  ...
  // Handle adding a new reminder with validation checks
  const handleAddReminder = async () => {
    // 1. Check if a date is selected
    if (!selectedDate) {
      alert("Please select a date.");
      return;
    }

    // 2. Parse the manual time input
    const [inputHours, inputMinutes] = manualTime.split(":").map((item) => parseInt(item, 10));
    const triggerDate = new Date(selectedDate);

    // 3. Set the time in triggerDate
    ...
    // 4. Ensure the time is in the future
    if (triggerDate <= new Date()) {
      alert("Please select a future time.");
      return;
    }

    // 5. Create the reminder object
    const newReminder = {
      id: Date.now(),
      date: selectedDate,
    ...
    // 6. Save the reminder, update state, schedule notification
    try {
  ...
  // Schedule background or system notifications based on platform OS
  ...
  // Delete a reminder from state and persist to local storage
  ...
  // Sub-component rendering individual reminders
  const Reminder = ({ item }) => (
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: isDarkMode ? COLORS.darkBackground : COLORS.lightWhite }}>
      ...
      <ScrollView contentContainerStyle={{ padding: SIZES.medium }}>
        ...
        {reminders.length > 0 ? (
          ...
          <Text style={{ color: isDarkMode ? COLORS.lightWhite : COLORS.darkText }}>No reminders yet.</Text>
        )}
      </ScrollView>
    </SafeAreaView>
  );
